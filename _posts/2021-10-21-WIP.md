...
<br/><br/>
And now let's replace all remaining "Nan" values from the combine with the average value for that metric <i>within</i> that player's position group. (for example, replace a "NaN" value for a cornerback's 40-yard dash with the average value of just the conernerback group. Here's the function to apply this:
<pre><code>
